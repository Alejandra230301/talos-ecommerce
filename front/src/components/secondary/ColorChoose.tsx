...
interface ColorChooseProps {
  color: IColor
  colorProducto: (title : string, image : string, name: string, stock:number) => void;
  product: string
}
...
const ColorChoose: React.FC<ColorChooseProps> = ({color, colorProducto, product}) => {
  ...
  return (
    <button onClick={() => colorProducto(title, image, name,stock)} className="rounded-full border border-slate-300 focus:border-slate-700 w-8 mx-1 my-2">
